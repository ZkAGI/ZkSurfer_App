...
function loadCentralKeypair(): Keypair {
  ...
  try {
    ...
    console.error("Key parsing error:", error);
    throw new Error("Failed to parse central wallet secret");
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!metadataUri || !nftName || !userPublicKey) {
    ...
    const metaplex = Metaplex.make(connection)
      .use(keypairIdentity(centralAuthority));

    // Fetch or create collection NFT
    const COLLECTION_NAME = "ZkAGI Community Mints";
    const myNfts = await metaplex.nfts().findAllByOwner({ owner: centralAuthority.publicKey });
    let collectionNft = myNfts.find(nft =>
      nft.name === COLLECTION_NAME &&
      nft.collectionDetails?.version === 'V1'
    );
    ...
    // Build the NFT creation and collection verification instructions
    ...
    // Sign with central authority and mint keypair
    transaction.sign(centralAuthority, newMintKeypair);
