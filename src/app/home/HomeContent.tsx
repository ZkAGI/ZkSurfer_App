    ...
    const fetchTickersAndInfo = async () => {
      if (wallet.publicKey) {
        try {
          ...
          console.log(
            "Updated availableTickers:",
            useTickerStore.getState().availableTickers
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (fullMessage.startsWith("/video-lipsync")) {
      ...
      if (videoFile && audioFile) {
        ...
        const videoUrl = await uploadFilesToMergeMedia(
          videoFile.file,
          audioFile.file
        );
    ...
    if (fullMessage.startsWith("/generate-tweet") && activeNavbarTicker) {
      ...
      const commandParts = fullMessage
        .replace("/generate-tweet", "")
        .trim()
        .split(" ");
      ...
      if (isNaN(numberOfTweets) || numberOfTweets <= 0 || numberOfTweets > 20) {
        const errorMessage: Message = {
          role: "assistant",
          content:
            "Please specify a valid number of tweets (1-20). Format: /generate-tweet [number] [prompt]",
          type: "text",
        };
        setDisplayMessages((prev) => [...prev, errorMessage]);
        return;
          ...
          const textGenResponse = await fetch("/api/chat", {
            ...
            body: JSON.stringify({
              messages: [
                {
                  role: "system",
                  content: `You are a helpful AI assistant trained to create content based on a given knowledge base. Using the example tweets provided in ${JSON.stringify(
                    twitterData
                  )}, learn the unique tone, style, and personality reflected in the writing, such as humor, formality, common themes, and favorite topics. Generate exactly ${numberOfTweets} new tweets in a style inspired by the given tweets, but ensure the wording, topics, and expressions are unique.
...
In addition to the tweets, use ${JSON.stringify(
                    trainingData
                  )} as supplementary knowledge for topics to include in the tweets. Keep responses strictly in these formats.`,
                },
                {
                  role: "user",
                  content: [
          ...
          try {
            ...
            const tweetContent =
              textGenData.content.match(/"tweet":\s*"([^"]*)"/g);
            if (tweetContent) {
          ...
          const generatedTweet = textGenDataContent.tweets.map(
            (item: { tweet: any }, index: number) => ({
              tweet: item.tweet,
              id: index + 1,
            })
          );
          ...
          try {
            ...
            promptMatch = textGenData.content.match(
              /"prompt"\s*:\s*"([^"]+)"/
            )?.[1];
          }
          ...
          if (promptMatch) {
            ...
            const tickerUserPrompt = tickerInfoUse
              ? tickerInfoUse.user_prompt
              : "";

            const fullPrompt = tickerUserPrompt
              ? `${promptMatch} with character ${tickerUserPrompt}`
              : promptMatch;
        ...
        const errorMessage: Message = {
          role: "assistant",
          content:
            "Sorry, there was an error processing your tweet generation request.",
          type: "text",
        };
        setDisplayMessages((prev) => [...prev, errorMessage]);
      }
    ...
    if (fullMessage.startsWith("/save") || fullMessage.startsWith("/saves")) {
      ...
      if (generatedTweets.length === 0) {
        const errorMessage: Message = {
          role: "assistant",
          content:
            "No generated tweets found. Please generate tweets first using /generate-tweet command.",
          type: "text",
        };
        setDisplayMessages((prev) => [...prev, errorMessage]);
        return;
      ...
      try {
        ...
        if (command.startsWith("/save ")) {
          ...
          const response = await fetch(
            "https://zynapse.zkagi.ai/contentengine_generatedtweet",
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          const tweetsToSave = tweetIds
            .filter((tweetId) => {
              if (
                isNaN(tweetId) ||
                tweetId < 0 ||
                tweetId >= generatedTweets.length
              ) {
        ...
        const errorMessage: Message = {
          role: "assistant",
          content: `Error: ${
            error instanceof Error ? error.message : "Failed to save tweet(s)"
          }`,
          type: "text",
        };
        setDisplayMessages((prev) => [...prev, errorMessage]);
            ...
            <button
              onClick={async () => {
                ...
                try {
                  addMessage({
                    role: "user",
                    content: "generate a character.json using the above data",
                  });
                  const { messages } = useConversationStore.getState();
                          ...
                          {useFormStore.getState().success ? (
                            ...
                            <>
                              <p className="text-white mb-2">
                                Please review and confirm your character.json:
                              </p>
                              <CharecterJsonEditor
                    ...
                    {sampleCommands.map((cmd, index) => (
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-300">
                          ...
                          command. You need to enter your custom instruction and
                          press enter or click submit to send your input prompt
                          for execution.
                        </p>
                      </div>
                    ))}
                ...
                <div>
                  ...
                  {displayMessages.map((message, index) => (
                    <div key={index} className="mb-4 flex justify-start w-full">
                      <div
                        key={index}
                        className="mb-4 flex justify-start w-full"
                      >
                  ...
                  {isLoading &&
                    (processingCommand ? (
                      ...
                      <div className="text-center">
                        ...
                        <p>
                          Processing your query. This may take up to 5
                          minutes...
                        </p>
                      </div>
                    ))}
