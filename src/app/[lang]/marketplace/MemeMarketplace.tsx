...
import { FC, useState, useCallback, useEffect, useRef, useMemo } from 'react';
import { Search, ArrowLeft, Store, Coins, Sparkles, Grid, List, TrendingUp } from 'lucide-react';
import { CoinCard } from '@/component/ui/CoinCard';
import { Coin, ApiCoin, ApiResponse } from '@/types/marketplaceTypes';
import { useWallet } from '@solana/wallet-adapter-react';
            ...
            <div className="ds-topbar sticky top-0 z-10">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="ds-heading-md">AI Coin Marketplace</h1>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-6 py-8">
                ...
                <div className="text-center mb-10">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full
                                    bg-dsPurple/10 border border-dsPurple/20 mb-6">
                        <Coins size={14} className="text-dsPurple-light" />
                        <span className="text-sm font-dmMono text-dsPurple-light">Deployed AI Coins</span>
                    </div>
                    <h2 className="ds-heading-lg mb-4">
                        Explore <span className="ds-gradient-text">AI Coins</span>
                    </h2>
                    <p className="ds-body max-w-2xl mx-auto mb-4">
                        Discover AI-powered tokens created on ZkTerminal. Each coin represents
                        an autonomous AI agent with unique capabilities.
                    </p>
                    <p className="text-xs text-dsMuted/70 max-w-xl mx-auto italic">
                        Disclaimer: ZkAGI provides tools and infrastructure but does not endorse
                        or guarantee the value of any coins listed.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                    <div className="ds-stats-panel">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-sm text-dsMuted">Total Coins</span>
                            <Coins size={14} className="text-dsPurple-light" />
                        </div>
                        <span className="text-white font-semibold font-dmMono">{filteredCoins.length}</span>
                    </div>

                    <div className="ds-stats-panel">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-sm text-dsMuted">Active Trades</span>
                            <TrendingUp size={14} className="text-dsGreen" />
                        </div>
                        <span className="text-white font-semibold font-dmMono">—</span>
                    </div>

                    <div className="ds-stats-panel">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-sm text-dsMuted">Platform</span>
                ...
                {!isLoading && (
                    <div className="flex items-center justify-between mb-6">
                        <p className="ds-body text-sm">
                            Showing <span className="text-white font-medium">{filteredCoins.length}</span> coins
                        </p>
                    </div>
                )}
                ...
                {!isLoading && filteredCoins.length === 0 && (
                    <div className="empty-state py-16">
                        <div className="w-20 h-20 rounded-2xl bg-dsBorder/30 flex items-center justify-center mb-6">
                            <Coins size={36} className="text-dsMuted/50" />
                        </div>
                        <h4 className="empty-state-title">No Coins Found</h4>
                        <p className="empty-state-description">
                            {searchQuery
                                ? `No coins match "${searchQuery}". Try a different search term.`
                                : 'No AI coins are available at the moment. Check back later!'}
                        </p>
                        {searchQuery && (
                            <button
                                onClick={() => setSearchQuery('')}
