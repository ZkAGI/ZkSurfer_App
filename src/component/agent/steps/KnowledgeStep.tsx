...
export default function KnowledgeStep() {
  ...
  // 👇 decide default per file from KB visibility
  const defaultDisclose = data.isKbPublic === false; // private KB => true, public KB => false
  ...
  for (const f of picked) {
    ...
    if (!existing.has(k)) {
      next.push(f);
      existing.set(k, f);
      // 👇 default Yes/No now follows the KB choice
      nextDisclosures.push({ key: k, name: f.name, disclose: defaultDisclose });
    }
  }
