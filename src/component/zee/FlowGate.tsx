...
// function VideoTile({
//   src, label, onClick, poster, disabled = false,
// }: { src: string; label: string; onClick: () => void; poster?: string; disabled?: boolean }) {
//   return (
//     <button
...
//       className={`group rounded-2xl p-2 bg-black/20 border border-white/10 transition-transform duration-200 ease-out
//         ${disabled ? 'opacity-60 cursor-not-allowed' : 'hover:border-white/20 hover:scale-[1.03] focus:outline-none'}`}
//     >
//       <div className="flex flex-col items-center gap-3">
...
//             className="h-full w-full object-cover"
//           />
//           <span className="absolute bottom-2 right-2 text-xs px-2 py-1 rounded bg-black/60 border border-white/10">
...
function VideoTile({
  ...
  return (
    <button
      ...
      aria-disabled={disabled}
      tabIndex={disabled ? -1 : 0}
      className={`group rounded-2xl p-2 bg-black/20 border border-white/10 transition-transform duration-200 ease-out w-full h-full
        ${disabled ? 'opacity-60 cursor-not-allowed' : 'hover:border-white/20 hover:scale-[1.03] focus:outline-none'}`}
    >
      <div className="flex flex-col items-center gap-3 w-full h-full">
        <div className="w-full h-full rounded-xl overflow-hidden relative">
          <video
            ...
            className="h-full w-full object-cover"
          />
          <span className="absolute bottom-2 right-2 text-xs px-2 py-1 rounded bg-black/60 border border-white/10 whitespace-nowrap">
            {label}{disabled ? ' (Coming Soon)' : ''}
          </span>
        </div>
      </div>
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className="relative z-[120] w-full max-w-4xl h-[90vh] max-h-[600px] bg-[#0D0F1E]/95 border border-[#283056] rounded-2xl p-4 sm:p-6 shadow-2xl flex flex-col"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="flex items-center justify-between mb-4 flex-shrink-0">
            <h2 className="text-lg sm:text-xl md:text-2xl font-bold">Start building your Zero Employee Enterprise</h2>
            <button className="text-sm opacity-70 hover:opacity-100" onClick={() => setFlowGateOpen(false)}>
              Close
            </button>
          </div>

          {/* Video container - takes up available space */}
          <div className="flex-grow min-h-0 mb-4">
            <div className="grid grid-cols-2 gap-3 sm:gap-4 h-full">
              {/* Video containers maintain 9:16 ratio within available height */}
              <div className="relative h-full">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-full max-w-[calc(100vh*9/16*0.5)] max-h-full aspect-[9/16]">
                    <VideoTile src="/videos/enterprise.mp4" label="ENTERPRISE" onClick={handleEnterprise} />
                  </div>
                </div>
              </div>
          ...
          <div className="flex-shrink-0">
            <div className="text-center text-xs sm:text-sm text-gray-300 mb-2">60 seconds to ZEE</div>
            <div className="flex justify-center">
              <button
                className="px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base rounded-full bg-gradient-to-r from-fuchsia-500 to-purple-500 hover:opacity-90"
                onClick={handleEnterprise}
              >
                BUILD YOUR SWARM
              </button>
